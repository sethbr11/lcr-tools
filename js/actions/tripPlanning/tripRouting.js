(() => {
  async function getMapboxTravelMatrix(points, apiKey) {
    ...
    for (let i = 0; i < numPoints; i += MAX_COORDS_PER_REQUEST) {
      for (let j = 0; j < numPoints; j += MAX_COORDS_PER_REQUEST) {
        const sourcesBatch = points.slice(i, Math.min(i + MAX_COORDS_PER_REQUEST, numPoints));
        const destinationsBatch = points.slice(j, Math.min(j + MAX_COORDS_PER_REQUEST, numPoints));
        ...
        if (i === j && sourcesBatch.length === destinationsBatch.length) { // Square sub-matrix
          allBatchPoints = sourcesBatch;
          if (allBatchPoints.length < 2) continue; // Skip if too few points for a square matrix batch
          sourceIndices = Array.from({length: allBatchPoints.length}, (_, k) => k);
          destinationIndices = Array.from({length: allBatchPoints.length}, (_, k) => k);
        } else { // Rectangular sub-matrix
          ...
          sourceIndices = Array.from({length: sourcesBatch.length}, (_, k) => k);
          destinationIndices = Array.from({length: destinationsBatch.length}, (_, k) => sourcesBatch.length + k);
        }

        url = `https://api.mapbox.com/directions-matrix/v1/mapbox/driving/${allBatchPoints.map(p => `${p.lon},${p.lat}`).join(";")}/?access_token=${apiKey}&annotations=duration&sources=${sourceIndices.join(';')}&destinations=${destinationIndices.join(';')}`;
  ...
  function solveTspWithMatrix(matrix, points, startPoint = null, targetEndPoint = null) {
    if (points.length < 2)
      return { orderedPoints: points, totalDistance: 0 };
  ...
  async function optimizeRoutes() {
    ...
    try {
      ...
      const startingAddress = document.getElementById("startingAddressInput").value.trim();
      ...
      if (startingAddress) {
        ...
        geocodedStartingPoint = await tripGeocoding.geocodeAddressMulti(startingAddress, provider, apiKey);
        if (!geocodedStartingPoint) {
          tripUtils.log(`❌ Could not geocode starting address: ${startingAddress}. Please check the address.`);
          alert(`Could not geocode starting address: "${startingAddress}". Please check the address.`);
        ...
        tripUtils.log(`✅ Starting address geocoded: [${geocodedStartingPoint.lat.toFixed(6)}, ${geocodedStartingPoint.lon.toFixed(6)}]`);
      }
      ...
      const sortedClusterIds = Object.keys(clusterCentroids).sort((a, b) => {
          return clusterCentroids[b].geometry.coordinates[1] - clusterCentroids[a].geometry.coordinates[1];
      });
      ...
      for (let i = 0; i < sortedClusterIds.length; i++) {
        ...
        if (i < sortedClusterIds.length - 1) {
          ...
          const turfPoints = nextClusterPoints.map(p => turf.point([p.lon, p.lat]));
      ...
      tripUtils.log("❌ An error occurred during optimization. See console for details.");
      optimizeBtn.disabled = false;
      optimizeBtn.textContent = "Optimize Routes";
    }
  }
  ...
  async function findBestInternalRoute(points, metric, apiKey, startingPoint = null, targetEndPoint = null) {
    ...
    if (currentStartPoint && !effectivePoints.some(p => p.lat === currentStartPoint.lat && p.lon === currentStartPoint.lon)) {
    ...
    if (metric === "mapbox" && apiKey) {
      try {
        ...
        const result = solveTspWithMatrix(matrix, effectivePoints, currentStartPoint, targetEndPoint);
        routePoints = result.orderedPoints;
      } catch (err) {
        console.error("Matrix optimization failed, falling back to NN:", err);
        routePoints = await solveTspNearestNeighbor(effectivePoints, currentStartPoint, targetEndPoint);
      }
    } else {
      routePoints = await solveTspNearestNeighbor(effectivePoints, currentStartPoint, targetEndPoint);
    }
    ...
    const wasAdded = currentStartPoint && !points.some(p => p.lat === currentStartPoint.lat && p.lon === currentStartPoint.lon);
    
    if (wasAdded && routePoints.length > 0 && routePoints[0].lat === currentStartPoint.lat && routePoints[0].lon === currentStartPoint.lon) {
        routePoints.shift();
    }
    ...
    for (let i = 0; i < routePoints.length - 1; i++) {
      totalDist += await getDistance(routePoints[i], routePoints[i + 1], metric, apiKey);
    }
  ...
  async function solveTspNearestNeighbor(points, startPoint = null, targetEndPoint = null) {
    ...
    if (startPoint && unvisited.some(p => p.lat === startPoint.lat && p.lon === startPoint.lon)) {
      currentPoint = unvisited.find(p => p.lat === startPoint.lat && p.lon === startPoint.lon);
    } else if (startPoint) {
